-- Simple fix to ensure race starts after countdown
-- Remove all complex logic and make it straightforward

-- Simplified race tick function that WILL start racing after countdown
...
BEGIN
  ...
  -- Handle COUNTDOWN state (5 second countdown) - FORCE START RACING
  SELECT * INTO current_race
  FROM race_state 
  ...
  IF current_race IS NOT NULL THEN
    ...
    -- FORCE START RACING after 5 seconds - NO CONDITIONS, JUST START
    IF countdown_duration_ms >= 5000 THEN
      -- FORCE the race to start - update state to 'racing'
      UPDATE race_state 
      SET 
        race_state = 'racing',
      ...
      RAISE NOTICE 'FORCING RACE START NOW! State changed to racing at %', current_time;
      
      -- Also log to confirm the change happened
      IF FOUND THEN
        RAISE NOTICE 'SUCCESS: Race state updated to racing in database';
      ELSE
        RAISE NOTICE 'ERROR: Failed to update race state to racing';
      END IF;
    END IF;
  ...
  -- Handle RACING state - horses actually move
  SELECT * INTO current_race
  FROM race_state 
  ...
  IF current_race IS NULL THEN
    RAISE NOTICE 'No racing state found - waiting for race to start';
    RETURN;
  END IF;

  RAISE NOTICE 'RACE IS ACTIVE - Moving horses! Race started at %', current_race.race_start_time;
  ...
  -- Update horse positions - FORCE MOVEMENT
  FOR horse_record IN 
    SELECT * FROM horses 
  ...
  LOOP
    ...
    -- FORCE position update - horses MUST move
    new_position := LEAST(
    ...
    RAISE NOTICE 'Horse % position: % -> % (velocity: %)', 
      horse_record.name, 
      COALESCE(horse_record.position, 0), 
      new_position, 
      current_velocity;
