-- Fix race completion to wait for all horses to finish before ending race

-- Update the race tick function to wait for all horses to finish
...
BEGIN
  ...
  -- Check if ALL horses have finished OR race has been running too long (safety timeout)
  IF finished_horses >= total_horses OR race_duration_ms > 60000 THEN -- 60 second timeout
