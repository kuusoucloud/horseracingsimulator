-- Fix countdown timer by ensuring countdown_start_time is always set
...
BEGIN
  ...
  CASE current_race.race_state
    ...
    WHEN 'countdown' THEN
      -- If countdown_start_time is NULL, set it now (for existing races)
      IF current_race.countdown_start_time IS NULL THEN
        UPDATE race_state 
        SET countdown_start_time = NOW()
        WHERE id = current_race.id;
        -- Use NOW() as reference for this calculation
        countdown_elapsed := 0;
      ELSE
        -- Countdown phase (3 seconds) - use countdown_start_time
    ...
    WHEN 'racing' THEN
      -- If race_start_time is NULL, set it now (for existing races)
      IF current_race.race_start_time IS NULL THEN
        UPDATE race_state 
        SET race_start_time = NOW()
        WHERE id = current_race.id;
        elapsed_time := 0;
      ELSE
        -- Race in progress - use race_start_time
        elapsed_time := EXTRACT(EPOCH FROM (NOW() - current_race.race_start_time))::INTEGER;
      END IF;
