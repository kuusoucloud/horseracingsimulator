-- Add results countdown timer to race_state for RaceController to handle

-- Add results_countdown field to race_state table
ALTER TABLE race_state 
ADD COLUMN IF NOT EXISTS results_countdown INTEGER DEFAULT 0;

-- Update the race tick function to handle results countdown in RaceController style
    ...
    CASE current_race.race_state
        ...
        WHEN 'racing' THEN
            ...
            IF finished_horses >= total_horses OR (current_race.race_timer + 0.1) >= 30 THEN
                ...
                RAISE NOTICE 'Race finished, starting 15-second results countdown in RaceController';
            END IF;
        ...
        WHEN 'finished' THEN
            ...
            IF current_race.results_countdown > 0 THEN
                ...
                RAISE NOTICE 'Results countdown: % seconds remaining', current_race.results_countdown - 1;
            ELSE
                -- Results countdown finished, start new race
                RAISE NOTICE 'Results countdown finished, starting new race';
                PERFORM start_new_race();
            END IF;
