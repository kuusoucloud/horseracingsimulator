-- Fix stack depth limit exceeded by removing nested DECLARE blocks
...
DECLARE
  ...
  base_speed NUMERIC;
  stamina_factor NUMERIC;
  random_variation NUMERIC;
  acceleration_boost NUMERIC;
  progress NUMERIC;
  finished BOOLEAN;
  updated_horse JSONB;
BEGIN
  ...
  ELSIF current_race.race_state = 'racing' THEN
    ...
    LOOP
      ...
      ELSE
        -- Calculate horse progress without nested DECLARE
        current_position := COALESCE((horse->>'position')::NUMERIC, 0);
        base_speed := COALESCE((horse->>'speed')::NUMERIC, 0.5) * 15;
        stamina_factor := GREATEST(0.5, COALESCE((horse->>'stamina')::NUMERIC, 0.5));
        random_variation := (RANDOM() - 0.5) * 8 * stamina_factor;
        acceleration_boost := CASE WHEN new_timer < 5 THEN COALESCE((horse->>'acceleration')::NUMERIC, 0.5) * 5 ELSE 0 END;
        progress := LEAST(1200, current_position + base_speed + random_variation + acceleration_boost);
        finished := progress >= 1200;
    ...
    IF finished_count >= jsonb_array_length(current_race.horses) OR race_timeout THEN
      ...
      WITH sorted_horses AS (
        SELECT 
          h.value as horse_data,
          ROW_NUMBER() OVER (
            ORDER BY 
              COALESCE((h.value->>'finishTime')::INTEGER, 999),
              (h.value->>'position')::NUMERIC DESC
          ) as placement
        FROM jsonb_array_elements(updated_horses) as h
      )
      SELECT jsonb_agg(horse_data || jsonb_build_object('placement', placement))
      INTO updated_horses
      FROM sorted_horses;
      
      -- Get top 3 horses for results
      WITH top_horses AS (
        SELECT h.value as horse_data
        FROM jsonb_array_elements(updated_horses) h
        ORDER BY (h.value->>'placement')::INTEGER
        LIMIT 3
      )
      SELECT jsonb_agg(horse_data) INTO top_3_results FROM top_horses;
      ...
      SET 
        ...
        race_results = top_3_results,
        show_results = true,
        show_photo_finish = true,
        photo_finish_results = top_3_results
      WHERE id = current_race.id;
    ELSE
      UPDATE race_state 
