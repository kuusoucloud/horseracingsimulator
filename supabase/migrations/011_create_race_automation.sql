-- Create a race automation system that doesn't rely on client-side edge function calls

-- First, let's create a simple race control table
...
INSERT INTO race_control (is_active) VALUES (true) ON CONFLICT DO NOTHING;

-- Create a function to advance the race state
...
BEGIN
  ...
  UPDATE race_control SET last_tick = NOW();
END;
$$;

-- Enable realtime for race_control table
ALTER PUBLICATION supabase_realtime ADD TABLE race_control;