-- Fix ELO-based odds calculation to create more realistic and varied odds
...
BEGIN
  ...
  -- If all horses have same ELO, add some variation for realistic odds
  IF elo_range < 50 THEN
    FOR i IN 1..array_length(all_elos, 1) LOOP
      all_elos[i] := all_elos[i] + (random() * 100 - 50)::INTEGER;
    END LOOP;
    SELECT MIN(elo), MAX(elo) INTO min_elo, max_elo FROM unnest(all_elos) AS elo;
    elo_range := GREATEST(1, max_elo - min_elo);
  END IF;
  ...
  FOR i IN 1..array_length(all_elos, 1) LOOP
    ...
    normalized_strength := power(2, normalized_strength * 3); -- Creates 1x to 8x range
    total_strength := total_strength + normalized_strength;
  END LOOP;
  
  -- Generate horse data with improved ELO-based odds
  FOR i IN 1..array_length(selected_names, 1) LOOP
    ...
    normalized_strength := power(2, normalized_strength * 3);
    ...
    -- Generate attributes based on ELO with more variation
