-- Restore race logic while keeping it simple to avoid stack depth issues
...
BEGIN
  ...
  -- Handle racing state with simple horse movement
  ELSIF current_race.race_state = 'racing' THEN
    new_timer := COALESCE(current_race.race_timer, 0) + 1;
    
    -- Simple race completion after 20 seconds with basic horse movement
    IF new_timer >= 20 THEN
      ...
      SET 
        ...
        horses = '[
          {"id": "1", "name": "Thunder Bolt", "position": 1200, "placement": 1, "finishTime": 18},
          {"id": "2", "name": "Lightning Strike", "position": 1200, "placement": 2, "finishTime": 19},
          {"id": "3", "name": "Storm Chaser", "position": 1200, "placement": 3, "finishTime": 20},
          {"id": "4", "name": "Wind Runner", "position": 1180, "placement": 4, "finishTime": 20},
          {"id": "5", "name": "Fire Dash", "position": 1160, "placement": 5, "finishTime": 20}
        ]'::jsonb,
        race_results = '[
