-- Restore the working automatic race system that was functioning before

-- Restore the original working auto_restart_race function
...
BEGIN
  ...
  IF current_race IS NULL THEN
    -- No finished race found, check if we need to start the very first race
    IF NOT EXISTS (SELECT 1 FROM race_state WHERE race_state IN ('pre-race', 'countdown', 'racing')) THEN
  ...
  IF time_since_finish > 15 THEN
    -- Check if there's already an active race to prevent duplicates
    IF NOT EXISTS (SELECT 1 FROM race_state WHERE race_state IN ('pre-race', 'countdown', 'racing')) THEN
...
-- Restore the original working race tick function that calls auto-restart every tick
...
DECLARE
  ...
  horse_result JSONB;
BEGIN
  -- First check if we need to auto-restart a finished race (this was working before)
