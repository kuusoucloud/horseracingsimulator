-- Fix race tick function errors

-- Fix the tick count calculation that's causing the max(uuid) error
...
BEGIN
  -- Use timestamp-based tick counting instead of max(uuid) which doesn't work
  seconds_since_epoch := EXTRACT(EPOCH FROM current_time)::INTEGER;
  
  -- Only check for auto-restart every 5 seconds
  IF seconds_since_epoch % 5 = 0 THEN
