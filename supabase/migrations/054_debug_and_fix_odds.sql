-- Debug and fix odds calculation with proper ELO handling
...
BEGIN
  ...
  -- Get ELO statistics for debugging
  SELECT MIN(elo), MAX(elo), AVG(elo) INTO min_elo, max_elo, avg_elo 
  FROM unnest(all_elos) AS elo;
  
  -- Log ELO range for debugging
  RAISE NOTICE 'ELO Range: Min=%, Max=%, Avg=%', min_elo, max_elo, avg_elo;
  
  -- Use a more conservative ELO scaling to prevent extreme odds
  -- Instead of 10^(ELO/400), use a dampened version: 10^(ELO/800)
  total_strength := 0;
  FOR i IN 1..array_length(all_elos, 1) LOOP
    -- Dampen the ELO effect to prevent extreme odds
    horse_strength := power(10, all_elos[i]::DECIMAL / 800.0);
    total_strength := total_strength + horse_strength;
    RAISE NOTICE 'Horse % ELO=% Strength=%', i, all_elos[i], horse_strength;
  END LOOP;
  
  RAISE NOTICE 'Total Strength: %', total_strength;
  
  -- Generate horse data with dampened ELO-based odds
  FOR i IN 1..array_length(selected_names, 1) LOOP
    current_horse_name := selected_names[i];
    horse_elo := all_elos[i];
    
    -- Calculate this horse's strength and probability using dampened ELO
    horse_strength := power(10, horse_elo::DECIMAL / 800.0);
    ...
    RAISE NOTICE 'Horse % (%) ELO=% Probability=% RawOdds=%', i, current_horse_name, horse_elo, probability, odds_value;
    ...
    -- Ensure reasonable bounds - cap at 50 instead of 999
    odds_value := GREATEST(1.01, LEAST(50.0, odds_value));
    
    RAISE NOTICE 'Final odds for %: %', current_horse_name, odds_value;
