-- Add countdown_start_time and race_start_time columns to race_state table
ALTER TABLE race_state 
ADD COLUMN IF NOT EXISTS countdown_start_time TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS race_start_time TIMESTAMP WITH TIME ZONE;

-- Fix the countdown timer infinite loop by using created_at for countdown timing
  ...
  CASE current_race.race_state
    WHEN 'pre-race' THEN
      ...
      IF elapsed_time >= 10 THEN
        -- Start countdown phase - set a countdown_start_time
        UPDATE race_state 
        SET 
    ...
    WHEN 'countdown' THEN
      -- Countdown phase (3 seconds) - use countdown_start_time if available, otherwise updated_at
      countdown_elapsed := EXTRACT(EPOCH FROM (NOW() - COALESCE(current_race.countdown_start_time, current_race.updated_at)))::INTEGER;
      ...
      IF countdown_elapsed >= 3 THEN
        ...
        WHERE id = current_race.id;
      ELSE
        -- Update countdown (don't update updated_at to avoid resetting the timer)
        UPDATE race_state 
        SET 
          countdown_timer = GREATEST(0, 3 - countdown_elapsed)
        WHERE id = current_race.id;
      END IF;

    WHEN 'racing' THEN
      -- Race in progress - use race_start_time if available, otherwise updated_at
      elapsed_time := EXTRACT(EPOCH FROM (NOW() - COALESCE(current_race.race_start_time, current_race.updated_at)))::INTEGER;
      
      -- Update horse positions based on their attributes
      FOR horse IN SELECT * FROM jsonb_array_elements(current_race.horses)
      LOOP
        ...
        -- Update horse position in the horses array
        current_race.horses := jsonb_set(
          current_race.horses,
          ('{' || (jsonb_array_elements_index(current_race.horses, horse)) || ',position}')::text[],
          to_jsonb(new_position)
        );
        
        -- Check if horse finished (reached 1200m)
      ...
      UPDATE race_state 
      SET 
        horses = current_race.horses,
        race_timer = elapsed_time,
        updated_at = NOW()
      WHERE id = current_race.id;
      
      -- Check if race is finished (all horses finished or 30 seconds elapsed)
      IF jsonb_array_length(finish_times) >= jsonb_array_length(current_race.horses) OR elapsed_time >= 30 THEN
        ...
        LOOP
          race_results := race_results || jsonb_build_object(
            'id', temp_result->>'name',
        ...
        -- Update ELO ratings
        PERFORM update_horse_elo_after_race(race_results);
        ...
        SET 
          ...
          photo_finish_results = race_results,
          updated_at = NOW()
        WHERE id = current_race.id;
      END IF;
