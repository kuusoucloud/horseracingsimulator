...
function FollowCamera({
  ...
  useFrame(({ camera }, delta) => {
    ...
    const interpolationSpeed = isRacing ? 20.0 : 10.0; // Ultra-fast camera tracking
...
function SmoothHorse({
  ...
  useFrame((state, delta) => {
    ...
    const interpolationSpeed = isRacing ? 25.0 : 12.0; // Ultra-fast interpolation
    const maxDelta = 0.016; // Cap delta to prevent jumps (60fps)

    // Calculate desired position with ultra-fast, buttery-smooth interpolation
    const cappedDelta = Math.min(delta, maxDelta);
    const lerpFactor = Math.min(cappedDelta * interpolationSpeed, 1.0);
